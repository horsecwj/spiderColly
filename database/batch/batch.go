...
// WithDatabase Select the current default database
...
// WithReadTimeout Timeout in second
...
// WithWriteTimeOut Timeout in second
...
// WithBlockSize Maximum rows in block (default is 1000000).
// If the rows are larger then the data will be split into several blocks to send them
// to the server
...
func New(insertSql string, opts ...Option) (*BatchInsert, error) {
	...
	//tcp://127.0.0.1:9000?block_size=1000&database=ethDatas&password=123456&read_timeout=10&username=default&write_timeout=20
	url := GetUrl(options)
	...
	for {
		...
		if l.Len() > 0 {
			...
			if err != nil {
				...
				common.Logger.Info("flush error", err)
			} else {
